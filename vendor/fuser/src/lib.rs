...
// FUSE-T's NFS backend behaves like libfuse2; the macFUSE-only flags
// (CASE_INSENSITIVE / VOL_RENAME / XTIMES) confuse it, so keep the plain
// libfuse2-compatible flag set when built against FUSE-T.
